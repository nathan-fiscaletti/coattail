...
class Token extends Model {
    ...
    static async issue(config, {database, validBearers, notBefore, expiresIn, performable, publishable, subscribable, subscribedTo}={}) {
    ...
    static getTokenIssuer(config) {
        return `${config.server.listen.address}:${config.server.listen.port}`;
    }
    ...
    async tryAuthenticate(config, connection, sourceIp, signature, extraVerifications={}) {
