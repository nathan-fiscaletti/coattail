...
module.exports = class extends Operation {
    ...
    async handle(client) {
        try {
            const {authenticationTokenId, validationToken, action, receiver, peerId} = this.data;
            ...
            const token = await Token.issueSubscription({authenticationTokenId, validationToken: vt, subscribedTo: [action], receiver, peerId});
