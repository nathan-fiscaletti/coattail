...
const log = (prefix, color=true, stdOut=process.stdout, stdErr=process.stderr) => {
    ...
    return {
        ...
        format: function (level, msg) { return `${new Date().toISOString()} ${level} ${this.prefixString()} ${typeof msg === 'object' ? (this.color ? JSON.colorStringify(obj, null, 2) : JSON.stringify(obj, null, 2)) : msg}`; },
