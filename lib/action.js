...
class Action {
        ...
        return new Promise((resolve, reject) => {
            self.validateInput(input).then(validationRes => {
                ...
                self.perform(input).then(output => {
                    if (publish) {
                        const database = db();
                        self.publishWithOutputValidation(database, output).then(() => {
                            resolve(output);
                        }).catch(reject).finally(() => database.destroy());
                        return;
                    }
                    resolve(output);
    ...
    publishWithOutputValidation(database, output) {
        return new Promise((resolve, reject) => {
