...
class Client extends Connection {
    static connect(peer, logger=log('Client')) {
        ...
        if (config.tls) {
            socket = tls.connect(config.port, config.address);
        } else {
            socket = net.connect(config.port, config.address);
        }
    ...
    constructor(config, logger, socket) {
        ...
        this.socket.on('end', () => {
            this.logger.warning(`closed connection to ${this.config.address}:${this.config.port}`);
            this.logger = this.baseLogger;
        });
    }

    _authenticate() {
        ValidationToken.getSignature(require(`./config`).load()).then(signature => {
            Operations.get(`auth`, {
