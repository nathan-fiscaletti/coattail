...
module.exports = async cli => {
    ...
    Subscription.loadAll().then(subscriptions => {
        const data = [[chalk.hex('#4e88e6').bold('Subscription ID'), chalk.hex('#4e88e6').bold('Connection'), chalk.hex('#4e88e6').bold('Subscribed To'),  chalk.hex('#4e88e6').bold('Authentication Token ID')]];
        ...
        if (subscriptions.length > 0) {
            ...
            cli.success(
                `Subscribers`,
                chalk.hex('#e64e4e')('No subscribers issued.')
            );
        }
    });
};