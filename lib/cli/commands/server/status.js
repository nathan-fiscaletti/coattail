...
const options = [
    ...
    },
    {
        name: 'config',
        alias: 'c',
        description: 'The path to the configuration file used by the server. Defaults to the config.yml file stored in the root of your Coattail installation.',
...
module.exports = async cli => {
    ...
    let procs = await findProcess('port', config.get().server.listen.port);
    for (const proc of procs) {
        proc.port = config.get().server.listen.port;
        processes.push(proc);
    }
