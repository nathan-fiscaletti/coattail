...
module.exports = async cli => {
    ...
    peer.getActions().then(actions => {
        if (!actions.some(a => a.name === parameters.action)) {
            ...
            const [ action ] = actions.filter(a => a.name === parameters.action);
            
            if (!isEqual(action.outputSchemaCompiled, receiver.inputSchemaCompiled)) {
                const outputSchema = JSON.colorStringify(action.outputSchemaCompiled, null, 2).split(/\r?\n/);
                const inputSchema = JSON.colorStringify(receiver.inputSchemaCompiled, null, 2).split(/\r?\n/);
                cli.error(
                    `The output schema of action '${action.name}' does not match the input schema of receiver '${receiver.name}'.`,
                    [
                        `'${action.name}' Compiled Output Schema`,
                        '',
                        ...outputSchema,
                        '',
                        `'${receiver.name}' Compiled Input Schema`,
                        '',
                        ...inputSchema
                    ]
                );
            ...
            peer.subscribeTo(parameters).then(({token, peerId}) => {
                let lines = [];
                token.print({showId: true, extras: {"Peer Token ID": peerId}, outputHandler: _lines => lines = _lines});
                cli.success(
                ...
                cli.error(`Failed to subscribe to peer.`, error.stack);
            });
        }
    }).catch(err => {
