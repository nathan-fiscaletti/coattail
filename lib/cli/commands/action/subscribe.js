...
module.exports = async cli => {
    ...
    try {
        ...
        console.log(err);
        console.error(`${chalk.hex('#e64e4e')('Error')}: No action found with name '${parameters.receiver}'. Please provide a valid action for the receiver.`);
        return;
    }
    ...
    peer.subscribeTo(parameters)
        .then(({token, peerId}) => {
            console.log('');
            console.log(` ${chalk.hex('#6ce64e')('Success')}: Subscribed to action '${chalk.hex('#6ce64e')(parameters.action)}' on peer '${chalk.hex('#4e88e6')(parameters.peer)}'.`);
