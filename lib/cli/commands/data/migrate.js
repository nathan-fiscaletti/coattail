...
module.exports = async cli => {
    ...
    database.migrate[parameters.action](config.get().data.migrations).then(([batchNo, log]) => {
        if (parameters.action === 'latest') {
            if (log.length === 0) {
                cli.success(chalk.hex('#6ce64e')('Already up to date.'));
                return;
            }
    
            cli.success(
                chalk.hex('#6ce64e')(`Action '${chalk.hex('#4e88e6')(parameters.action)}' complete.`),
                chalk.hex('#6ce64e')(`Batch ${batchNo} run: ${log.length} migrations:${os.EOL}${log.join(os.EOL)}`)
            );
        } else if (parameters.action === 'rollback') {
            if (log.length === 0) {
                cli.success(chalk.hex('#6ce64e')('Already at the base migration.'));
                return;
            }
