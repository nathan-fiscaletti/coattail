...
module.exports = async cli => {
    ...
    const versionPath = path.join(directory, '.ct.version');
    ...
    const install = path.resolve(path.join(__dirname, '..', '..'));
    ...
    try {
        fs.writeFileSync(versionPath, package.version);
        lines.push(`${chalk.hex('#4e88e6')(versionPath)}       : ${chalk.hex('#6ce64e')(`Success!`)}`);
    } catch (err) {
        printFailed(chalk.hex('#4e88e6')(versionPath), err);
    }
    ...
    try {
        templates.create('config', configPath, 'yml', {
            ...
            '{dbfile}': dbPath,
            '{install}': install,
            '{actions}': path.join(directory, 'actions'),
            '{receivers}': path.join(directory, 'receivers'),
            '{log}': logPath,
            '{migrations}': path.join(install, 'lib', 'data', 'migrations'),
            '{auth-key-priv}': authPrivateKeyPath,
            '{auth-key-pub}': authPublicKeyPath,
            '{validation-key-priv}': vtPrivateKeyPath,
            '{validation-key-pub}': vtPublicKeyPath
        });
    ...
    database.migrate['latest'](config.get().data.migrations).then(_ => {
        lines.push(``);
        lines.push(`${chalk.hex('#4e88e6')('Running database migrations...')}`);
        cli.success(`Coattail Instance Created`, lines);
    }).catch(error => {
        printFailed('Failed to run migrations.', error);
    }).finally(() => {
        database.destroy();
    });
};