...
module.exports = async cli => {
    ...
    try {
        ...
        console.log(` ${chalk.hex('#4e88e6')(actionsPath)}      : ${chalk.hex('#6ce64e')(`Success!`)}`);
    } catch (err) {
        console.log(` ${chalk.hex('#4e88e6')(actionsPath)}      : ${chalk.hex('#e64e4e')(`${err}`)}`);
        console.log('');
        return;
    }
    ...
    try {
        ...
        console.log(` ${chalk.hex('#4e88e6')(logPath)}  : ${chalk.hex('#6ce64e')(`Success!`)}`);
    } catch (err) {
        console.log(` ${chalk.hex('#4e88e6')(logPath)}  : ${chalk.hex('#e64e4e')(`${err}`)}`);
        console.log('');
        return;
    }
    ...
    const proc = childProcess.spawn('node', [paths.CMD_PATH, 'data', 'migrate', 'latest'], {
        cwd: directory,
        detached: true,
